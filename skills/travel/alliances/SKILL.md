...
**Reference data:** `~/.hermes/skills/travel/data/alliances.json`
...
Verify against `~/.hermes/skills/travel/data/alliances.json` for current state. These have shifted recently:
...
The best redemptions often involve booking an airline through a program in a DIFFERENT alliance (or no alliance at all). Always check the `cross_alliance_highlights` section of `~/.hermes/skills/travel/data/alliances.json` and `~/.hermes/skills/travel/data/partner-awards.json`.
...
For the full picture of which programs ticket which airlines, plus bilateral partnerships outside alliances, load the `partner-awards` skill or read `~/.hermes/skills/travel/data/partner-awards.json` directly.
