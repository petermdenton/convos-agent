...
Data lives in [`~/.hermes/skills/travel/data/stopovers.json`](../../~/.hermes/skills/travel/data/stopovers.json). Refresh cadence is 90 days because stopover rules change rarely (much less often than transfer bonuses or program valuations).
...
The full data is in `~/.hermes/skills/travel/data/stopovers.json`. Headlines:
...
For full details, restrictions, recent changes, and primary sources for any program, query `~/.hermes/skills/travel/data/stopovers.json`.
...
jq '.programs[] | select(.category == "positive") |
    ...
    ~/.hermes/skills/travel/data/stopovers.json | head -40
...
jq '.programs[] | select(.category == "positive" and .surcharge_points == 0 and .booking_method == "phone") |
    ...
    ~/.hermes/skills/travel/data/stopovers.json

# Programs that do NOT allow stopovers (negative space)
jq '.programs[] | select(.category == "negative") | {name, restrictions}' ~/.hermes/skills/travel/data/stopovers.json

# All sources/citations for a given program
jq '.programs[] | select(.id == "aeroplan") | .sources' ~/.hermes/skills/travel/data/stopovers.json

# Recently changed programs (last 24 months)
jq '.programs[] | select(.recent_changes != null) | {name, recent_changes}' ~/.hermes/skills/travel/data/stopovers.json

# Discontinued programs (e.g. United Excursionist)
jq '.programs[] | select(.discontinued == true) | {name, discontinued_date, restrictions}' ~/.hermes/skills/travel/data/stopovers.json
```
