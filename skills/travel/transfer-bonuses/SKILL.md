...
`~/.hermes/skills/travel/data/transfer-bonuses.json`. Schema (relevant fields):
...
When the user asks "what bonuses are active right now?", read `~/.hermes/skills/travel/data/transfer-bonuses.json` and produce a markdown table. Sample format (illustrative — pull live values from the data file, not from this example):
...
(The table format and notes apply across program-pairs. The actual current end dates and effective ratios live in `~/.hermes/skills/travel/data/transfer-bonuses.json` and update weekly via `scripts/refresh-transfer-bonuses.py`.)
