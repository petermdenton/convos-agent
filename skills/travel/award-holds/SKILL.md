...
Data lives in [`~/.hermes/skills/travel/data/award-holds.json`](../../~/.hermes/skills/travel/data/award-holds.json). Refresh cadence is 90 days because hold rules change rarely.
...
jq '.programs[] | select(.category == "positive") |
    ...
    ~/.hermes/skills/travel/data/award-holds.json | head -50

# Programs that do NOT allow holds
jq '.programs[] | select(.category == "negative") | {name, restrictions}' \
    ~/.hermes/skills/travel/data/award-holds.json
...
jq '.transfer_speed_reference' ~/.hermes/skills/travel/data/award-holds.json
```
