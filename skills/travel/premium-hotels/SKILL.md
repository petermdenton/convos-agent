...
jq --argjson lat $LAT --argjson lng $LNG --argjson r $R \
  ...
  ) | {name, program, credit}]' ~/.hermes/skills/travel/data/fhr-properties.json

jq --argjson lat $LAT --argjson lng $LNG --argjson r $R \
  '[.properties[] | select(
    .coordinates.lat > ($lat - $r) and .coordinates.lat < ($lat + $r) and
    .coordinates.lng > ($lng - $r) and .coordinates.lng < ($lng + $r)
  ) | {name, program, credit}]' ~/.hermes/skills/travel/data/thc-properties.json
```
...
) | {name, location}]' ~/.hermes/skills/travel/data/chase-edit-properties.json
```
...
FHR=$(jq --argjson lat $LAT --argjson lng $LNG --argjson r $R \
  '[.properties[] | select(
    .coordinates.lat > ($lat - $r) and .coordinates.lat < ($lat + $r) and
    .coordinates.lng > ($lng - $r) and .coordinates.lng < ($lng + $r)
  ) | {name, program: "FHR", credit}]' ~/.hermes/skills/travel/data/fhr-properties.json)
...
THC=$(jq --argjson lat $LAT --argjson lng $LNG --argjson r $R \
  '[.properties[] | select(
    .coordinates.lat > ($lat - $r) and .coordinates.lat < ($lat + $r) and
    .coordinates.lng > ($lng - $r) and .coordinates.lng < ($lng + $r)
  ) | {name, program: "THC", credit}]' ~/.hermes/skills/travel/data/thc-properties.json)

# Chase Edit
CHASE=$(jq '[.properties[] | select(
  (.location // "" | test("Oslo|Norway"; "i")) or
  (.name // "" | test("Oslo"; "i"))
) | {name, program: "Chase Edit", credit: "See Chase portal for current credit"}]' ~/.hermes/skills/travel/data/chase-edit-properties.json)
...
- `~/.hermes/skills/travel/data/fhr-properties.json` (1,807 properties, coordinates + Amex reservation links + Google Travel price calendar links + credit details)
- `~/.hermes/skills/travel/data/thc-properties.json` (1,299 properties, coordinates + Amex reservation links)
- `~/.hermes/skills/travel/data/chase-edit-properties.json` (1,553 properties, text locations, 190 tagged `budget_friendly` from "Potentially Cheaper Ones" category)
