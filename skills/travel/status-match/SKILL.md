...
**Reference data:** [`~/.hermes/skills/travel/data/status-match.json`](../../~/.hermes/skills/travel/data/status-match.json). Refresh cadence is 90 days because rules change occasionally but not weekly.
...
For full details, see `direct_matches` section of `~/.hermes/skills/travel/data/status-match.json`.
...
For full details, see `paid_concierge` section of `~/.hermes/skills/travel/data/status-match.json`.
...
For full details, see `card_based_status` section of `~/.hermes/skills/travel/data/status-match.json`.
...
Query `card_based_status` in `~/.hermes/skills/travel/data/status-match.json`. If a card already grants the equivalent tier and the user holds (or would hold) that card, recommend that path. **No status match needed.**
...
jq '.direct_matches.airlines | to_entries[] | select(.value.fee == "free") | {program: .value.program, lifetime: .value.lifetime_warning}' ~/.hermes/skills/travel/data/status-match.json

# All paid concierge programs with US fees
jq '.paid_concierge.programs | to_entries[] | {program: .value.program, us_fee: (.value.fees_by_region.us // .value.fees_summary // null)}' ~/.hermes/skills/travel/data/status-match.json

# All cards that grant hotel status
jq '.card_based_status | to_entries[] | select(.value.renewable_status != null) | {card: .value.card, annual_fee: .value.annual_fee_usd, status: [.value.renewable_status[] | .program + " " + .tier]}' ~/.hermes/skills/travel/data/status-match.json

# All ONCE PER LIFETIME programs (never miss this warning)
jq '[.direct_matches.airlines, .direct_matches.hotels, .paid_concierge.programs] | add | to_entries[] | select((.value.lifetime_warning // "") | test("LIFETIME"; "i")) | {program: .value.program, warning: .value.lifetime_warning, detail: .value.lifetime_warning_detail}' ~/.hermes/skills/travel/data/status-match.json
```
