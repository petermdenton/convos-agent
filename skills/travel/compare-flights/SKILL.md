...
For every seats.aero award result, calculate the effective cost in each transferable currency using `~/.hermes/skills/travel/data/transfer-partners.json`:
...
For each award (e.g., United business at 88,000 miles):
  ...
  3. Calculate opportunity cost using ~/.hermes/skills/travel/data/points-valuations.json:
...
| `~/.hermes/skills/travel/data/points-valuations.json` | CPP floor/ceiling for calculating opportunity cost |
| `~/.hermes/skills/travel/data/partner-awards.json` | Cross-alliance booking options (VA→ANA, EY→AA, etc.) |
